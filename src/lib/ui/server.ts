...
export interface UiServerOptions {
  ...
  setDefaultSandbox?: (sandboxName: string) => boolean;
...
function cliCommandLine(args: string[]): string {
  return [CLI_NAME, ...args.map(displayCliArg)].join(" ");
}
  ...
  if (parts[0] === "api" && parts[1] === "sandboxes" && typeof parts[2] === "string") {
    ...
    if (req.method === "GET" && parts.length === 4 && parts[3] === "endpoint") {
      ...
      if (!sandbox.isDefault) {
        sendJson(res, 409, {
          error: `Sandbox '${sandboxName}' is not the default sandbox. Make it default before opening its endpoint.`,
        });
        return;
      }
    ...
    if (req.method === "POST" && parts.length === 4 && parts[3] === "default") {
      if (!validateSandboxForRequest(sandboxName, res, opts.sandboxExists)) return;
      await readJsonBody(req);
      const overview = await opts.overviewProvider();
      const sandbox = overview.sandboxes.find((entry) => entry.name === sandboxName);
      ...
      if (sandbox.isDefault) {
        sendJson(res, 200, {
          ...
          status: 0,
          commands: [],
          stdout: `'${sandboxName}' is already the default sandbox.`,
          stderr: "",
        });
        return;
      }
      const changed = opts.setDefaultSandbox(sandboxName);
      sendJson(res, changed ? 200 : 404, {
        ok: changed,
        status: changed ? 0 : 1,
        commands: [],
        stdout: changed ? `Default sandbox changed to '${sandboxName}'.` : "",
        stderr: changed ? "" : `Could not set '${sandboxName}' as the default sandbox.`,
      });
      return;
    }
...
function normalizeOptions(options: UiServerOptions): Required<UiServerOptions> {
  ...
  return {
    ...
    setDefaultSandbox: options.setDefaultSandbox ?? ((sandboxName) => registry.setDefault(sandboxName)),
